...
public class Memory {
	...
	public void writedata(PCB pcb, String varname, String data) {
		int start = (int) this.memory[(pcb.processID - 1) * 5 + 4];
		int end = (int) this.memory[(pcb.processID - 1) * 5 + 4] - 3; 
	...
	public String readFromMemory(PCB p, String varname) {
		String reString = "";
		int start = (int) this.memory[(p.processID - 1) * 5 + 4];
		int end = (int) this.memory[(p.processID - 1) * 5 + 4] - 3;
		for (int i =start; i > end; i--)
		{
